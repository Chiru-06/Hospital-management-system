...
const Dashboard: React.FC = () => {
    ...
    const fetchStats = async () => {
      try {
        ...
        setStats({
          ...
          monthlyRevenue: billingResponse.data
            .filter((bill: any) => {
              const billDate = new Date(bill.date);
              return billDate.getMonth() === currentMonth && billDate.getFullYear() === currentYear;
            })
            .reduce((total: number, bill: any) => total + (bill.amount || 0), 0),
  ...
  const statCards = [
    ...
    },
    {
      ...
      icon: <AttachMoney sx={{ fontSize: 30, color: theme.palette.secondary.main }} />,
      ...
      <Grid container spacing={3} sx={{ mt: 3 }}>
        ...
        <Grid item xs={12} md={4}>
          <StatCard>
            <CardContent>
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
                <AccessTime sx={{ fontSize: 30, color: theme.palette.primary.main, mr: 1 }} />
                <Typography variant="h6" sx={{ fontWeight: 'bold' }}>
                  Recent Activities
                </Typography>
              </Box>
